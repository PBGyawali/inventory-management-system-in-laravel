<div class="btn-group text-center">
    ...
    @if (in_array('reset', $buttons))
        <li><button type="button"   id="{{$id}}" data-id="{{$id}}"class="w-100 mb-1 text-center btn btn-warning btn-sm reset"><i class="fas fa-sync"></i> Reset</button></li>
    @endif
      <li><button type="button" name="update" id="{{$id}}" data-id="{{$id}}" data-prefix="{{trim(ucwords($prefix.' '.($extratext??'')))}}" class="w-100 mb-1 text-center btn btn-info btn-sm update"><i class="fas fa-edit"></i> Update</button></li>
