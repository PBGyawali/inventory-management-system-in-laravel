...
<table  cellpadding="4" cellspacing="0" class="text-center border border-solid mb-5" >
    <tbody>
       <tr>
          <td colspan="5" class="text-center text-red-600 uppercase underline text-2xl">CuSTOMER BILL</td>
       </tr>
       <tr >
          <td rowspan="4"class="border-l border-solid  border-black " colspan="2" >{{$info->company_logo!=''? '<img src="'.$info->company_logo.'" class="ml-2 mb-2 rounded-circle"alt="logo image not found " width="120px;"  >':''}}</td>
          <td colspan="3" class="text-right pr-3" >ORIGINAL</td>
       </tr>

       <tr >
          <td colspan="3"  class="text-right text-red-600 underline text-2xl fontsemi-bold  pr-3" >{{$info->company_name}}</td>
       </tr>
       <tr>
       ...
       <tr>
          <td colspan="6" class="text-right text-primary underline pr-3">{{$info->company_email}}</td>
       </tr>
       <tr>
 ...
 <tr class= h-6 border-b border-l border-black border-solid" >
       ...
       <tr class= h-6 border-b border-black border-solid" >
        <td colspan="3" class="pl-1 text-left border-solid  border  border-black ">Payment Bank :-{{$info->company_bank}} </td>
          <td class=" bg-black text-white text-center w-40 pl-2 border border-black border-solid">Total</td>
       ...
       <tr>
        <td colspan="3"  class="p-3 pl-1 text-left border border-solid border-black w-96">Branch :- {{$info->company_bank_address}} </td>
          <td rowspan="1" class=" bg-black text-white text-center pl-1 border border-black w-48" >Discount</td>
       ...
       <tr>
        <td colspan="3" class="pl-1 text-left border-b border-l border-black">Bank IBAN CODE :- {{$info->company_bank_IBAN}}</td>
          <td rowspan="1" class=" bg-black text-white text-center pl-1 border border-black w-36" >Tax</td>
          <td rowspan="1"  class="text-center  border border-black w-52" >{{$total_tax_amount}}
          </td>
       </tr>
       <tr>
        <td colspan="3" class="p-3 pl-1 text-left border-b border-l border-black " >AC Name :- {{$info->company_name}}</td>
          <td  class=" bg-black text-white text-center pl-1 border">G. Total</td>
          <td  class="text-center border" >{{$total}}</td>
       </tr>
       <tr  class="h-20">
          <td colspan="4" class="pl-1 text-left border-solid border-l border-r border-b  border-black">
          Amount in words : <span class=" border-b border-dotted border-gray-900"> ({{$info->company_currency}}) {{ $total_in_words }}  Only</span>
             </td>
          <td  colspan="2" ><b class="my-5 uppercase text-blue-600 border-solid  border border-purple-900 text-3xl">{{$info->company_name}}</b></td>
       </tr>
