$(document).ready(function()
{
    ...
    function updateGraph($category,$value,id=null,$type='line')
    {
	   ...
	   if (window['figure_'+id].chart!=undefined) {
        ...
        let backgroundColor=['line','bar'].includes($type)?'rgb(255, 99, 132)':['rgb(255, 99, 132)','rgb(87, 191, 46)','rgb(255, 205, 86)'];
        if ($space)
        {
    ...
    { 	var id=$(this).data('id');
        ...
        let chart=$('#bargraph'+'_'+id).data('chart');
        let value=$('#bargraph'+'_'+id).data('type');
      $.ajax
      ({
            url: url,
            method: 'post',
            data:  {table:table,type:type,value:value},
           dataType:"JSON",
            success: function(data)
            {
                let lables=data.labels;
                let datas=data.data;
                updateGraph(lables,datas,id,chart);
            }
        });
    });
